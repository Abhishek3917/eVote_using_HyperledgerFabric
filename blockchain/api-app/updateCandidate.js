...
async function main() {
    try {
        ...
        app.post('/updateCandidate', async (req, res) => {
            const { ID,electionName, candidateName, votes } = req.body;

            // Invoke the chaincode function
            try {
                const response = await contract.submitTransaction('UpdateCandidate',ID,electionName, candidateName, parseInt(votes));
